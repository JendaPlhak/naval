...
Dále ukážeme, že pro každé $k\in\{0,1,\dots,n\}$ je pravda, že po $k$-té iteraci cyklu for platí tvrzení

\begin{equation} \label{invariant}
\forall i\in \N, 1\leq i\leq k: A[i]\in S \Leftrightarrow (\forall j\in\N, i<j\leq k: A[i]>A[j]).
\end{equation}

To je zřejmě pravda pro $k=0$, resp. $k=1$ (v těchto případech je zásobník prázdný, resp. je v něm právě prvek $A[1]$). Stačí tedy ukázat, že z pravdivosti tohoto faktu pro $k$ vyplývá i jeho pravdivost pro $k+1$ (potom totiž bude platit i pro $n$ a formule výše nám zajistí korektnost celého algoritmu). Nejprve poznamenejme, že z indukčního předpokladu plyne, že prvky zásobníku tvoří klesající posloupnost (ode dna po vrchol). Z toho plyne, že while cyklus na řádcích 3-5 odstraní ze zásobníku všechny mrakodrapy, které nevidí přes prvek $A[k+1]$. Předpokládajme, že jsme odstranili alespoň jeden prvek ze zásobníku a poslední odstraněný mrakodrap ze zásobníku byl $A[l], 1\leq l\leq k$. Pak z předchozího vyplývá platnost následujících podmínek:
\begin{equation} \label{bagr}
\forall j\in\N, l< j < k + 1, A[j]\in S_k: A[j] \leq A[k+1],
\end{equation}
